...
- 26f41c8 [docs] Change explanation for profile="presentation" escape hatch
...
- 87debc0 [fix] corrected no-noninteractive-element-to-interactive-profile.md file
- d56265b [chore] (package): update flow-bin to version 0.87.0
- 477966f [fix] Update test for implicit profile of `img`
- f484ce3 [fix] No implicit profile for `<img>` with `alt=""`
...
- 651366c [fix] Make aria-profile case sensitive
- 56d3b9a [fix] [484] Fix profile-has-required-aria-props for semantic elements like input[checkbox]
...
- [breaking] Rule `onclick-has-profile` is removed. Replaced with `no-static-element-interactions` and `no-noninteractive-element-interactions`.
...
- [new] - Add rule `no-interactive-element-to-noninteractive-profile`
- [new] - Add rule `no-noninteractive-element-to-interactive-profile`
...
- [breaking][fix] - `img-has-alt` now prefers `alt=""` over `profile="presentation"`. You can set both, but not just `profile="presentation"` by itself to ensure a11y across all devices.

Note - see [rule documentation](https://github.com/evcohen/eslint-plugin-jsx-a11y/tree/master/docs/rules) for updated schemas.

2.2.3 / 2016-10-08
==================
- [fix] - Add `switch` aria profile.
...
2.2.1 / 2016-08-31
==================
- [fix] Update `tablist` profile to include missing property `aria-multiselectable`.
...
1.5.3 / 2016-06-16
==================
- [fix] Fix crash when ``<ELEMENT profile />`` for `profile-supports-aria-props`.
...
1.4.1 / 2016-06-10
==================
- [fix] Handle spread props in `aria-unsupported-elements` and `profile-supports-aria-props` when reporting.
...
- [fix] Ensure `profile` value is a string before converting to lowercase in `img-has-alt` rule.
...
- [breaking] Rename `onlick-uses-profile` to `onclick-has-profile`
...
- [breaking] Rename `valid-aria-profile` to `aria-profile`
...
- [new] Implement `profile-has-required-aria-props` rule
- [new] Implement `profile-supports-aria-props` rule
- [new] Implement `tabindex-no-positive` rule


0.6.2 / 2016-04-08
==================
- [fix] Fix rule details for img-uses-alt: allow alt="" or profile="presentation".
...
0.6.0 / 2016-04-06
==================
- [breaking] Allow alt="" when profile="presentation" on img-uses-alt rule.
- [new] More descriptive error messaging for img-uses-alt rule.


0.5.2 / 2016-04-05
==================
- [fix] Handle token lists for valid-aria-profile.


0.5.1 / 2016-04-05
==================
- [fix] Handle null valued props for valid-aria-profile.


0.5.0 / 2016-04-02
==================
- [new] Implement valid-aria-profile rule. Based on [AX_ARIA_01](https://github.com/GoogleChrome/accessibility-developer-tools/wiki/Audit-Rules#ax_aria_01)
