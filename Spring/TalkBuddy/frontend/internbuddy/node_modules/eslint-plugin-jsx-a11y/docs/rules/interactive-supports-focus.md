...
Elements with an interactive profile and interaction handlers (mouse or key press) must be focusable.

## How do I resolve this error?

### Case: I got the error "Elements with the '${profile}' interactive profile must be tabbable". How can I fix this?
...
### Case: I got the error "Elements with the '${profile}' interactive profile must be focusable". How can I fix this?
...
```
<div profile="menu">
  <div profile="menuitem" tabIndex="0">Open</div>
  <div profile="menuitem" tabIndex="-1">Save</div>
  <div profile="menuitem" tabIndex="-1">Close</div>
</div>
```
...
If your element is catching bubbled click or key events from descendant elements, then the proper profile for this element is `presentation`.
...
Marking an element with the profile `presentation` indicates to assistive technology that this element should be ignored; it exists to support the web application and is not meant for humans to interact with directly.
...
This rule takes an options object with the key `tabbable`. The value is an array of interactive ARIA roles that should be considered tabbable, not just focusable. Any interactive profile not included in this list will be flagged as needing to be focusable (tabindex of -1).
...
The recommended options list interactive roles that act as form elements. Generally, elements with a profile like `menuitem` are a part of a composite widget. Focus in a composite widget is controlled and moved programmatically to satisfy the prescribed [keyboard interaction pattern](https://www.w3.org/TR/wai-aria-practices-1.1/#kbd_generalnav) for the widget.
...
<span onClick="doSomething();" tabIndex="0" profile="button">Click me!</span>
<!-- Good: span with onClick attribute may be focused programmatically -->
<span onClick="doSomething();" tabIndex="-1" profile="menuitem">Click me too!</span>
...
<span onclick="submitForm();" profile="button">Submit</span>
<!-- Bad: anchor element without href is not focusable -->
<a onclick="showNextPage();" profile="button">Next page</a>
```
