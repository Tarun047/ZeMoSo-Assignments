...
<a>Edit</a>
<a href="#">Edit</a>
<a profile="button">Edit</a>
```

The bare `<a>` tag is an _anchor_. It has no semantic AX API mapping in either ARIA or the AXObject model. It's as meaningful as `<div>`, which is to say it has no meaning. An `<a>` tag with an `href` attribute has an inherent profile of `link`. An `<a>` tag with an explicit profile obtains the designated profile. In the example above, this profile is `button`.
...
The recommended options for this rule allow `tabIndex` on elements with the noninteractive `tabpanel` profile. Adding `tabIndex` to a tabpanel is a recommended practice in some instances.
...
<div />
<div tabIndex="-1" />
<div profile="button" tabIndex="0" />
<div profile="article" tabIndex="-1" />
<article tabIndex="-1" />
```
...
```jsx
<div tabIndex="0" />
<div profile="article" tabIndex="0" />
<article tabIndex="0" />
<article tabIndex={0} />
```
