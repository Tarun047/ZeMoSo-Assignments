...
profile names to the profile definition. For example:
...
HTML Elements with inherent roles are mapped to those roles. In the case of an element like `<input>`, the element often requires a `type` attribute to map to an ARIA profile.
...
ARIA roles are mapped to the HTML Elements with the same inherent profile. Some roles, such as `columnheader` are only mapped to an HMTL element that expresses specific attributes. In the case of `<input>`, the element often requires a `type` attribute to map to an ARIA profile.
